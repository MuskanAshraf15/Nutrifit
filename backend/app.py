...
def update_profile():
    ...
    try:
        ...
        # MySQL can report 0 affected rows when submitted values are unchanged.
        db.commit()
        return jsonify({
            "success": True,
            "message": "Profile updated successfully",
...
def recommend_food():
    ...
    try:
        ...
        if selected_budget == "Low":
            ...
            budget_min = 601
            budget_max = 999999
        # Meal type and preference are hard filters; goal, activity and budget are used for scoring.
        cursor.execute("""
            SELECT
        ...
        # Start a new recommendation cycle after all temporary choices have been used.
        # Foods marked as disliked stay excluded from the next cycle.
...
def contact():
    ...
    try:
        db = get_db_connection()
        cursor = db.cursor()
        # Contact messages can also be submitted without a logged-in user.
        user_id = get_jwt_identity()
        query = """
...
def admin_add_food():
    ...
    try:
        ...
        serving_value = float(data.get("Serving_g"))
        estimated_cost_value = float(data.get("Estimated_Cost"))
        protein_value = float(data.get("Protein_g") or 0)
        carbs_value = float(data.get("Carbs_g") or 0)
        fat_value = float(data.get("Fat_g") or 0)
    except (TypeError, ValueError):
        return jsonify({
            "success": False,
            "message": "Calories, serving and estimated cost must be valid numbers"
        }), 400
    if calories_value <= 0:
        return jsonify({
            "success": False,
    ...
    if serving_value <= 0:
        return jsonify({
            "success": False,
            "message": "Serving must be greater than 0 grams"
        }), 400
    ...
    if protein_value < 0 or carbs_value < 0 or fat_value < 0:
        return jsonify({
            "success": False,
            "message": "Protein, carbs and fat cannot be negative"
        }), 400
    db = None
    cursor = None
    try:
...
def admin_update_food(food_id):
    ...
    data = request.get_json() or {}
    try:
        calories_value = float(data.get("Calories"))
        serving_value = float(data.get("Serving_g"))
        estimated_cost_value = float(data.get("Estimated_Cost"))
        protein_value = float(data.get("Protein_g") or 0)
        carbs_value = float(data.get("Carbs_g") or 0)
...
def delete_admin_account():
    ...
    # Let the browser complete the CORS preflight before checking admin auth.
